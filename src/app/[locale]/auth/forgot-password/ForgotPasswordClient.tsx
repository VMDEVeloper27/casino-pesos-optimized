...
export default function ForgotPasswordClient({ locale }: ForgotPasswordClientProps) {
  ...
  const texts = {
    es: {
      ...
      description: 'No te preocupes, te ayudaremos a recuperar tu cuenta en segundos',
      emailSent: '¡Correo Enviado!',
      sentInstructions: 'Hemos enviado las instrucciones a:',
      spamNote: 'Si no recibes el correo en 5 minutos, revisa tu carpeta de spam o solicita un nuevo enlace.',
    ...
    en: {
      ...
      description: "Don't worry, we'll help you recover your account in seconds",
      emailSent: 'Email Sent!',
      sentInstructions: 'We have sent instructions to:',
      spamNote: 'If you don\'t receive the email in 5 minutes, check your spam folder or request a new link.',
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        setError(data.error || t.emailError);
      }
    } catch (err) {
        ...
        >
          ...
          <div className="relative bg-white rounded-3xl p-8 md:p-12 max-w-md w-full border border-green-500/20 shadow-xl">
            ...
            <div className="grid grid-cols-3 gap-3 mb-8">
              {[
                { icon: '⚡', text: locale === 'es' ? 'Rápido' : 'Fast' },
                { icon: '🔒', text: locale === 'es' ? 'Seguro' : 'Secure' },
                { icon: '✨', text: locale === 'es' ? 'Fácil' : 'Easy' }
              ].map((item, i) => (
                <motion.div
                  key={i}
            ...
            <form onSubmit={handleSubmit} className="space-y-6">
              ...
              <button
                ...
                aria-label={isLoading ? t.sending : t.sendInstructions}
              >
            ...
            <div className="mt-8 pt-8 border-t border-gray-200 space-y-3">
              ...
              <p className="text-black/40 text-center text-xs">
                {t.noAccount}{' '}
                <Link href={`/${locale}/auth/signup`} className="text-black/60 hover:text-green-600 transition-colors">
                  {t.signUpFree}
                </Link>
              </p>
            </div>
